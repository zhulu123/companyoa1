...
<script>
import { arrayToTree } from "../../assets/js/array.js";
export default {
  data() {
    return {
  ...
  methods: {
    ...
    addToAims() {
      ...
      this.$emit("left-change", 0);
      this.$emit("right-change", 0);
    },
    // 移除按钮
    removeToSource() {
      // 获取选中通过穿梭框的keys - 仅用于传送纯净的id数组到父组件同后台通信
    ...
    fromTreeChecked(nodeObj, treeObj) {
      ...
      this.$emit("left-change", leafCheckedNodes.length);
    },
    // 目标树选中事件 - 是否禁用穿梭按钮
    toTreeChecked(nodeObj, treeObj) {
      ...
      let arrayCheckedNodes = this.$refs["to-tree"].getCheckedNodes();
      let children__ = this.defaultProps.children || "children";
      let rightCheckedNodes = arrayCheckedNodes.filter(
        item => !item[children__] || item[children__].length == 0
      );
      this.$emit("right-change", rightCheckedNodes.length);
    },
    // 源数据 总全选checkbox
